const lightLaser = extend(BasicBulletType, {
  update(b){
		...
		if(b.timer.get(1, 5)){
			Damage.collideLine(b, b.team, this.hitEffect, b.x, b.y, b.rotation(), this.laserLength * 1.1, true);
		};
	},
	...
	draw(b, tile){
		...
		for(var s = 0; s < 4; s++){
			...
			for(var i = 0; i < 4; i++){
				Tmp.v1.trns(b.rotation() + 180.0, (lenscales[i] - 1.1) * 55.0);
				Lines.stroke((0.8 + Mathf.absin(Time.time, 1.7, 3.1)) * b.fout() * strokes[s] * tscales[i]);
				Lines.lineAngle(b.x + Tmp.v1.x, b.y + Tmp.v1.y, b.rotation(), this.laserLength * b.fout() * lenscales[i], false);
			}
		};
	}
});
  ...
  update(b){
	...
	hit(b, hitx, hity){
		...
		if(Mathf.chance(Time.delta * 0.2)){
			Tmp.v2.trns(b.rotation() + 90.0, Mathf.range(4.0));
			Lightning.create(b.team, Color.valueOf("959dc2"), 10, b.x + Tmp.v2.x, b.y + Tmp.v2.y, b.rotation(), 46);
		}
	},
