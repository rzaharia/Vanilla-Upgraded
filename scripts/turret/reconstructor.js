...
const boostedEffect = new Effect(18, e => {
...
const reconstructorLaser = extend(BasicBulletType, {
	...
	scanUnits(b){
		...
		for(var i = 0; i < this.searchAccuracy; i++){
			vec.trns(b.rotation(), (this.length / this.searchAccuracy) * i);
			vec.add(b.x, b.y);
				...
				if(unit != null){
					if(Mathf.within(vec.x, vec.y, unit.x, unit.y, radius) && unit.team != b.team && unit && !unit.dead){
						if(unit.health < Math.max(unit.maxHealth * 0.05, 85)){
							...
							newUnit.add();
							newUnit.maxHealth = lastUnit.maxHealth * 1.2
							newUnit.health = lastUnit.health + 50;
							newUnit.apply(boosted, Number.MAX_VALUE);
	...
	draw(b){
		const colors = [Color.valueOf("00008bff"), primeColor, primeColor, Color.white];
		...
		for(var s = 0; s < 4; s++){
			...
			for(var i = 0; i < 4; i++){
				...
				Lines.lineAngle(b.x + Tmp.v1.x, b.y + Tmp.v1.y, b.rotation(), this.length * b.fout() * lenscales[i], false);
...
const reconstructor = extendContent(PowerTurret, "reconstructor", {
	load(){
		this.super$load();
